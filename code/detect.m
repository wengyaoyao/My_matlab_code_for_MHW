...
temp_mhw=temp(:,:,time>=mhw_start & time<=mhw_end);%截取出要求热浪期间的温度片段

%% Calculating climatology and thresholds
%计算气候态和阈值
date_true=datevec(cli_start-vWindowHalfWidth:cli_end+vWindowHalfWidth);%生成气候态计算开始时间前5天到结束后5天的时间序列
date_true=date_true(:,1:3);%截取其中 年，月，日 三列信息

date_false = date_true;
date_false(:,1) = 2012;%将年份替换为虚假年份2012

fake_doy = day(datetime(date_false),'dayofyear');%将日期转化为一年中的第几天
ind = 1:length(date_false);%生成一个1到气候态数据集的长度的索引序列

mclim=NaN(size(temp,1),size(temp,2),366);%生成一个与输入温度数据大小一致的气候态矩阵
m90=NaN(size(temp,1),size(temp,2),366);%生成一个与输入温度数据大小一致的90百分位矩阵

for i=1:366
    if i == 60 %跳过2.29号（即下标为60）
        
    else
        ind_fake=ind;%虚假索引等于索引序列
        ind_fake(fake_doy==i & ~ismember(datenum(date_true),cli_start:cli_end))=nan;%满足虚假序列第i天，并且日期不在气候态计算区间内的赋值为NaN
        %找出DOY +- 窗口（5天）的所有样本；对于第i天，找出窗口前后5天的数据，且数据落在气候态计算区间内
    ...
    %根据vercentile只选择case中的一种运行
    switch vpercentile
        %如果vpercentile值为'matlab'
        case 'matlab'
